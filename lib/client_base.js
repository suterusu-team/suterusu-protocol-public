...
class ClientBase {
    ...
    */
    async init() {
        ...
        */
        this.suter.events.TransferSuccess({})
            .on('data', (event) => {
                ...
                event.returnValues['parties'].forEach((party, i) => {
                    if (bn128.pointEqual(account.publicKey(), party)) {
                        ...
                        web3.eth.getBlock(blockNumber).then(async (block) => {
                            if (that.epochBase == 0)
                                account._state = await account.update(await that._getEpoch(block.number));
                            else if (that.epochBase == 1)
                                account._state = await account.update(await that._getEpoch(block.timestamp));
                            else
                                throw new Error("Invalid epoch base.");
        ...
        */
        this.account = new SuterAccount();
        // First update to initialize the state.
        this.account._state = await this.account.update(await that._getEpoch());

        this.account.keypair = undefined;
        this.account.aesKey = undefined; 
    }
    ...
    async setEpochBase (epochBase) {
        ...
        let transaction = that.suter.methods.setEpochBase(epochBase)
                ...
                .on('receipt', async (receipt) => {
                    ...
                    console.log("Set epoch base successful. Client lastRollOver updated to: ", that.account._state.lastRollOver);
                })
                .on('error', (error) => {
    ...
    async setEpochLength (epochLength) {
        ...
        let transaction = that.suter.methods.setEpochLength(epochLength)
                ...
                .on('receipt', async (receipt) => {
                    ...
                    that.account._state.lastRollOver = await that._getEpoch();
                    that.account._state.nonceUsed = true;
                    console.log("Set epoch length successful. Client lastRollOver updated to: ", that.account._state.lastRollOver);
                })
                .on('error', (error) => {
    ...
    */
    async withdraw (destination, value, burnGasLimit) {
        ...
        console.log("updated state: balance: ", account.balance(), ", available: ", state.available);
        console.log("current epoch: ", await that._getEpoch());
        console.log("current account lastRollOver: ", account._state.lastRollOver);
        console.log("update state lastRollOver: ", state.lastRollOver);
        ...
        if (value > state.available) {
            ...
            return sleep(wait * that.epochUnitTime).then(() => that.withdraw(destination, value, burnGasLimit));
        }
        if (state.nonceUsed) {
        ...
        if (that.epochBase == 0) {
            ...
            if ((that.epochLength / 4) >= wait) {
                ...
                return sleep(wait * that.epochUnitTime).then(() => this.withdraw(destination, value, burnGasLimit));
            }
        }
        ...
        var transaction;
        if (!destination)
            transaction = that.suter.methods.burn(account.publicKeySerialized(), value, u, proof, encGuess); 
        else
            transaction = that.suter.methods.burnTo(destination, account.publicKeySerialized(), value, u, proof, encGuess);
        ...
        console.log("Server current epoch: ", await that.suter.methods.currentEpoch().call());
    ...
    async transfer (receiver, value, decoys, transferGasLimit) {
        ...
        */
        ...
        if (value > state.available) {
            ...
            return sleep(wait * that.epochUnitTime).then(() => that.transfer(receiver, value, decoys, transferGasLimit));
        }
        if (state.nonceUsed) {
