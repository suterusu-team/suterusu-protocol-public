...
class ClientBase {
    ...
    */
    ...
    async buildWithdrawArguments (destination, value) {
        var that = this;
        that.account.checkDefined();
        that.checkValue();
        var account = that.account;
        ...
        if (value > state.available) {
            ...
            return sleep(wait * that.epochUnitTime).then(() => that.buildWithdrawArguments(destination, value));
        }
        if (state.nonceUsed) {
    ...
    @return A promise that is resolved (or rejected) with the execution status of the deposit transaction.
    */
    async withdraw (destination, value, burnGasLimit = 3000000) {
        var that = this;
        if (!destination)
            destination = that.home;
        var args = await that.buildWithdrawArguments(destination, value);

        let transaction = that.suter.methods.burnTo(args.sink, args.y, args.unitAmount, args.u, args.proof, args.encGuess);
    ...
    async buildTransferArguments(receiver, value, decoys) {
        /*
        ...
        */
        ...
        if (value > state.available) {
            ...
            return sleep(wait * that.epochUnitTime).then(() => that.buildTransferArguments(receiver, value, decoys));
        }
        if (state.nonceUsed) {
    ...
    */
    async transfer (receiver, value, decoys, transferGasLimit) {
        var that = this;
        var args = await that.buildTransferArguments(receiver, value, decoys);
        ...
        let transaction = 
            that.suter.methods.transfer(args.C, args.D, args.y, args.u, args.proof)
