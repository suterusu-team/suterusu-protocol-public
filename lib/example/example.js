...
const run = async () => {
    ...
    const suter = await Promise.all([deployer.deploySuterERC20(cash, transfer, burn, 1, 12, 1), deployer.mintTestERC20Token(cash, 1000)]).then((results) => results[0].contractAddress);
