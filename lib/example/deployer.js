...
class Deployer {
    constructor(accounts) {
        ...
        this.deploySuterETH = (transfer, burn, epochBase, epochLength, unit) => {
            const contract = new web3.eth.Contract(SuterETH.abi);
            return new Promise((resolve, reject) => {
                contract.deploy({ data: SuterETH.bytecode, arguments: [transfer, burn, epochBase, epochLength, unit] }).send({ from: accounts[0], gas: 4700000 })
                    .on("receipt", (receipt) => {
        ...
        this.deploySuterERC20 = (cash, transfer, burn, epochBaes, epochLength, unit) => {
            const contract = new web3.eth.Contract(SuterERC20.abi);
            return new Promise((resolve, reject) => {
                contract.deploy({ data: SuterERC20.bytecode, arguments: [cash, transfer, burn, epochBase, epochLength, unit] }).send({ from: accounts[0], gas: 4700000 })
                    .on("receipt", (receipt) => {
